...
**There's a DB-like interface exposed on to front end, and the API endpoints seemed to have a _little_ security I had to workaround.** This is the easiest and first thing to check. Initially I tried using Firefox's 'Edit and Resend' feature to alter the requests, to change the parameters but making requests out of order was yielding strange responses: Firefox would say 3.34kB transferred (0B in size). Only requests made in the expected order would provide results.
